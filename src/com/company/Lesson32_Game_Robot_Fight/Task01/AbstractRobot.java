...
//придаем значение переменной name, также можно через Setter
    ...
    public BodyPart defense() {
        hitCount = (int) (Math.random() * 4 + 0); // Math.random() - возвр double, кастим в int,  +0 - с какого числа начать
    ...
    public BodyPart attack() {
        ...
        if (hitCount == 0) {
            attack = BodyPart.hand;
        } else if (hitCount == 1) {
            attack = BodyPart.leg;
        } else if (hitCount == 2) {
            attack = BodyPart.head;
        } else if (hitCount == 3) {
            attack = BodyPart.body;
        }
